...
static void
unshrink(struct graph *graph, struct vertex *srknode, int ecount, int *elist,
         double *ecap),
ghtree_free_node(struct ghnode *p), update_outmax(struct ghnode *n),
ghtree_print_work(struct ghnode *root);

static int
shrinkdown(struct ghtree *ghtree, struct graph *graph, int nodecount,
           struct vertex **nodeset, struct vertex *srknode, int *ecount,
           int *elist, double *ecap),
gh_recursive(struct cp_par *par, struct graph *graph, struct ghtree *ghtree,
             struct ghnode *n, int scount, struct vertex **special),
countdescendants(struct ghnode *n);
           ...
           struct ghtree **ghtree)
{
  ...
  *ghtree = ghtree_create();
  ghtree_add_nodes(*ghtree, graph->nv);
  ...
  (*ghtree)->root->cutval = MAXDOUBLE;

  special = malloc(graph->n3v * sizeof(struct vertex *));
  scount  = 0;
  for (v = graph->tail; v; v = v->next)
  {
    if (v->mark_aux == 1)
  ...
  rval = gh_recursive(par, graph, *ghtree, (*ghtree)->root, scount, special);
...
{
  int rval = 0;
  struct vertex **a_srknodes, **b_srknodes;
  struct vertex **a_special = NULL, **b_special = NULL;
  struct vertex *v;
  int acount, bcount, ascount, bscount;
  struct ghnode *a_cut, *b_cut;
  struct ghnode *parent, *child, *c, *nextc;
  double cutvalue;
  struct ghnode *newnode;
  struct vertex *anode     = NULL;
  struct vertex *bnode     = NULL;
  int *elist               = NULL;
  double *ecap             = NULL;
  struct vertex **cutverts = NULL;
  int i, ecount, cutcount;
  double amaxval, bmaxval;
  struct vertex *amaxv = NULL, *bmaxv = NULL, *oldmax;
  int oldmcount;
  ...
  while (anode->i == bnode->i)
    bnode = special[rand() % scount];

  /***********************************************************/
  /* Solve (s-t)-minimum cut*/
  /***********************************************************/
  rval = mincut_solve(graph, anode, bnode, &cutvalue, &cutverts, &cutcount);
  check_rval(rval, "mincut_solve failed", CLEANUP);
  ...
  for (i = 0; i < cutcount; i++)
  {
    ...
    for (j = 1; j < ghtree->n[cutverts[i]->i]->mcount; j++)
      ghtree->n[cutverts[i]->i]->members[j]->special->mark = graph->marker;
  }
  for (i = 0; i < scount; i++) special[i]->mark_aux = graph->marker;

  /* divide them up */
  a_srknodes = malloc((graph->n3v - cutcount) * sizeof(struct vertex *));
  a_special  = malloc((graph->n3v - cutcount) * sizeof(struct vertex *));
  b_srknodes = malloc(cutcount * sizeof(struct vertex *));
  b_special  = malloc(cutcount * sizeof(struct vertex *));
  acount = bcount = 0;
  ascount = bscount = 0;
  amaxval           = -MAXDOUBLE;
  bmaxval           = -MAXDOUBLE;
  ...
  for (c = n->child, n->child = NULL, n->nchild = 0; c; c = nextc)
  {
    ...
    if ((c->special->mark == graph->marker &&
        ...
        (c->special->mark != graph->marker && n->special->mark != graph->marker))
    {
  ...
  rval = shrinkdown(ghtree, graph, bcount, b_srknodes, b_cut->special, &ecount,
                    elist, ecap);
  check_rval(rval, "shrinkdown failed", CLEANUP);
  if (b_srknodes)
    free(b_srknodes);

  rval = gh_recursive(par, graph, ghtree, a_cut, ascount, a_special);
  if (rval)
    goto CLEANUP;
  ...
  rval = shrinkdown(ghtree, graph, acount, a_srknodes, a_cut->special, &ecount,
                    elist, ecap);
  check_rval(rval, "shrinkdown failed", CLEANUP);
  if (a_srknodes)
    free(a_srknodes);

  rval = gh_recursive(par, graph, ghtree, b_cut, bscount, b_special);
  if (rval)
    goto CLEANUP;
...
ghtree_create(void)
{
  struct ghtree *ghtree = malloc(sizeof(struct ghtree));
  if (!ghtree)
  {
...
ghtree_add_node(struct ghtree *ghtree)
{
  ...
  node = ghtree->n[ghtree->nn] = malloc(sizeof(struct ghnode));
  node->i                      = ghtree->nn++;
  node->parent                 = NULL;
...
ghtree_add_nodes(struct ghtree *ghtree, int nn)
{
  int i;
  struct ghnode *node;
  ghtree->n = malloc(nn * sizeof(struct ghnode *));

  for (i = 0; i < nn; i++)
  {
    node = ghtree->n[i] = malloc(sizeof(struct ghnode));
    node->i             = ghtree->nn++;
    node->in_max        = NULL;
    node->parent        = NULL;
    node->next_sibling  = NULL;
    ...
    node->nchild        = 0;
    node->ndesc         = 0;
    node->members       = malloc(nn * sizeof(struct ghnode *));
    node->members[0]    = node;
    node->mcount        = 1;
  }
...
void
ghtree_free(struct ghtree *ghtree)
{
  int i;
  if (ghtree)
  {
...
static void
ghtree_free_node(struct ghnode *n)
{
  if (n)
  {
    free(n->members);
    ...
    if (v->i != srknode->i)
    {
      ...
      if (v->i != srknode->i)
      {
        ...
        for (j = 0; j < ghtree->n[v->i]->mcount; j++)
          ghtree->n[srknode->i]->members[ghtree->n[srknode->i]->mcount + j] =
...
static void
update_outmax(struct ghnode *node)
{
  struct ghnode *child;
...
void
ghtree_print(struct ghtree *ghtree)
{
  if (ghtree)
  {
    printf("\nGOMORY-HU TREE\n");
...
ghtree_print_work(struct ghnode *root)
{
  ...
  printf("OutMax %d (%.2f), InMax %d (%.2f)", root->out_max->i, root->out_max->y,
         root->in_max->i, root->in_max->y);
  printf("\n");
