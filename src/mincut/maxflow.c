...
static void
set_labels(struct graph *graph, struct vertex *s, struct vertex *t,
           struct vertex **high, int *highest, struct vertex **level),
relabel_dist(int n3v, struct vertex **level, struct vertex *v),
add_to_active(struct vertex **high, int *highest, struct vertex *v),
backwards_bfs(struct graph *graph, struct vertex *t, struct vertex **high,
              int *highest, struct vertex **level, int marker);
...
maxflow_solve(struct graph *graph, struct vertex *s, struct vertex *t)
{
  ...
  level = malloc((graph->n3v + 1) * sizeof(struct vertex *));
  for (i = 0; i < graph->n3v; i++) level[i] = NULL;
  level[graph->n3v] = NULL;

  high = malloc(graph->n3v * sizeof(struct vertex *));
...
{
  int i;
  struct vertex *v, *next, *other;
  struct arc *e;
  int label;
  struct vertex *dummy = malloc(sizeof(struct vertex));
